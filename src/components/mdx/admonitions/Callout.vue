<template>
  <div
    class="not-prose rounded-lg border-2 border-light-gray bg-pale py-4 px-6 dark:border-gray dark:bg-inherit"
  >
    <Disclosure :defaultOpen="open" v-slot="{ open }">
      <DisclosureButton as="div" class="flex items-center justify-between">
        <span class="text-2xl font-semibold tracking-tight">{{ show }}</span>
        <IconFaChevronRight
      ...
      <DisclosurePanel as="div" class="mt-4">
        <slot />
      </DisclosurePanel>
    </Disclosure>
  </div>
