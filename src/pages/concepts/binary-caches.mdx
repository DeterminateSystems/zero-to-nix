...
Whenever Nix [realises] a [derivation], it realises the entire [closure] of the derivation, that is, each derivation that's required to build the derivation, plus the derivations required to build those derivations, and so on.
But at each step in the realisation process
