---
title: Derivations
snippet: Build instructions for Nix [packages](/concepts/package)
related: ["nix-store", "package", "realisation"]
readMore: [
  {
    title: "Derivations",
...
A Nix *derivation* is an instruction that Nix uses to [*realise*][realisation] a [package].
Derivations are created using the [Nix language](#nix-language).
A derivation can depend on any number of other derivations and produce one or more final [outputs](#outputs).
...
<Admonition type="success" title="Mental model" client:load>
