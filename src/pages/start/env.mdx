...
<Switch>
  <Specific lang="Go" client:only>
    Ordinarily when you run `which go` on a Unix system, you get a path like `/usr/bin/go`. Try
    running it in the Nix shell:
  ...
  <Specific lang="Rust" client:only>
    Ordinarily when you run `which cargo` on a Unix system, you get a path like `/usr/bin/cargo`. Try
    running it in the Nix shell:
