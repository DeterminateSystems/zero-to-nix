...
Congrats 🚀! You're now in a [Bash] environment that includes [jq], [curl], and [Git]. You may already have all three in your environment, but run these commands to see that something new is happening:
...
In all three cases, you're running a symlinked package in the [Nix store][store] and nothing from your global environment. This makes Nix shell environments [*hermetic*][hermeticity].
