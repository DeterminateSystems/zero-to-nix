...
If you have [Git] installed, check your `PATH` for it using `which git`.
...
As in the previous examples, you can run commands like `which python` and `which kubectl` to see that these tools are all discoverable in the [Nix store][store] and not somewhere like `/usr/bin`.
