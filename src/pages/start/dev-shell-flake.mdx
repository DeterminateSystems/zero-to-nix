...
To get started in your <Language client:load /> project, create an empty directory and initialize a [flake templates][templates] for your preferred language:
...
## Exploring a Nix development environment for <Language client:load />

One of the [flake outputs][outputs] of this Nix [flakes] is a [development environment][dev] for <Language client:load />.
...
- Nix looked at the `devShells` [flake outputs][outputs] in `flake.nix` to figure out which [Nix packages][packages] to include in the development environment (Nix specifically looked at the `buildInputs` array).
