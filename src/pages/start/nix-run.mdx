...
summary:
  [
    "Use the `nix run` command to [run](#run) a program from [Nixpkgs](/concepts/nixpkgs)",
...
In the last section, we installed Nix using [Harmonic].
...
The first time you run a program using `nix run` it may take a while.
That's because Nix needs to build the program's [package] from scratch and store it in the [Nix store][store].
...
* It used the `nixpkgs#cowsay` [flake reference][flakes] to pull in some Nix code and drilled into a specific [flake output][output] (more on this later).
* It built the [`cowsay` package][cowsay] and stored the result in the [Nix store][store].
* It ran the ran the executable at `bin/cowsay` from the `cowsay` package.
