...
<Admonition warning title="`legacyPackages` isn't legacy software" id="legacy-packages" client:load>
The `legacyPackages` attribute that you see in the search output is a bit misleading.
The [packages] prefaced with that aren't *legacy packages*
...
<Admonition info title="When to use `nix search` vs. `nix flake show`" id="which-search-command" client:load>
