...
>
  ...
  The [packages] prefaced with that aren't "legacy" packages; instead, [Nixpkgs] uses a special `legacyPackages` attribute to output packages instead of the usual `packages` output for reasons laid out
  [this code comment][legacy-comment].
</Admonition>
